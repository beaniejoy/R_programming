...
str(data) # 'data.frame':	248 obs. of  14 variables:

# 변수 선택
x <- data$level2 # 학력수준
y <- data$pass2 # 합격유무

# 범주형으로 바꿔야 chi-square 교차분석이 가능하다.
table(x)
# 고졸     대졸 대학원졸 
# 93       86       57 
table(y)
...
# 귀무가설 : 부모의 학력수준이 자녀의 대학 진학에 영향을 미치는가?
CrossTable(x=df$Level, y=df$Pass, chisq = TRUE)
# Chi^2 =  2.766951     d.f. =  2     p =  0.2507057 
# 검정통계량 : chi^2, d.f. = 2(자유량)
# 유의확률 : p = 0.2507057 >= 유의수준(0.05) (H0 채택; 전혀 연관이 없다.)
...
#<검정통계량 해석>
# X-squared = 14.2, df = 5
X-squared >= 11.071 # (카이제곱분포표) -> 기각

# => 검정통계량을 가지고 유의확률이 만들어짐

# (2) 선호도 분석(이름만 연구환경에 따라 달라지는 것이지 본질은 같다.)
#-----------------------------------------
# 귀무가설 : 기대치와 관찰치는 차이가 없다.(기각)
# 대립가설 : 기대치와 관찰치는 차이가 있다.(채택)
#-----------------------------------------
data <- textConnection(
  "스포츠음료종류  관측도수
  1   41
...
chisq.test(x, y)
# X-squared = 156.27, df = 68, p-value = 6.666e-09 < 0.05 (H0 기각/ HA 채택)

# [해설]
# 꽃의 종별로 꽃받침의 길이가 차이가 있다고 볼 수 있다.
